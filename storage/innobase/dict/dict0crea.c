...
#error
	...
	ptr = mem_heap_alloc(heap, 4);
	if (table->flags & (~DICT_TF_COMPACT & ~((unsigned long long)-1 << DICT_TF_BITS))) {
		ut_a(table->flags & DICT_TF_COMPACT);
		ut_a(dict_table_get_format(table) >= DICT_TF_FORMAT_ZIP);
		ut_a((table->flags & DICT_TF_ZSSIZE_MASK)
		     <= (DICT_TF_ZSSIZE_MAX << DICT_TF_ZSSIZE_SHIFT));
		ut_a(!(table->flags & ((unsigned long long)-1 << DICT_TF2_BITS)));
		mach_write_to_4(ptr, table->flags & ~((unsigned long long)-1 << DICT_TF_BITS));
	} else {
	...
	tab_node_t*	node)	/*!< in: table create node */
{
	...
	if (file_per_table) {
		...
		flags = table->flags & ~((unsigned long long)-1 << DICT_TF_BITS);
		...
		table->flags &= ((unsigned long long)-1 << DICT_TF_BITS) | DICT_TF_COMPACT;
	}
