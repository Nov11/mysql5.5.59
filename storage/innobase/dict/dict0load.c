...
# error "missing case labels for DICT_TF_FORMAT_ZIP .. DICT_TF_FORMAT_MAX"
	...
	if (UNIV_UNLIKELY(flags & ((unsigned long long)-1 << DICT_TF_BITS))) {
...
err_len:
	...
	if (UNIV_UNLIKELY(type & ((unsigned long long)-1 << DICT_IT_BITS))) {
...
err_len:
	...
	if (n_cols & 0x80000000UL) {
		...
		if (flags2 & ((unsigned long long)-1 << (DICT_TF2_BITS - DICT_TF2_SHIFT))) {
			ut_print_timestamp(stderr);
			...
			flags2 &= ~((unsigned long long)-1 << (DICT_TF2_BITS - DICT_TF2_SHIFT));
		}
	...
	if (table->space == 0) {
		...
		if (table->flags & (DICT_TF2_TEMPORARY << DICT_TF2_SHIFT)) {
			...
			if (!fil_open_single_table_tablespace(
				...
				table->flags & ~((unsigned long long)-1 << DICT_TF_BITS), name)) {
